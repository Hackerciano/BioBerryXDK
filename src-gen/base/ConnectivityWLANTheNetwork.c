...
#define NETWORK_SSID  "iPhone de Alejandro"
#define NETWORK_PSK  "del 1 al 8"

static void ConnectivityWLANTheNetwork_WlanConnectStatusCallback(WlanNetworkConnect_Status_T connectStatus)
{
	BCDS_UNUSED(connectStatus);
	(void) xSemaphoreGive(WlanEventSemaphore);
}
static void ConnectivityWLANTheNetwork_NetworkIpConfigStatusCallback(WlanNetworkConfig_IpStatus_T ipStatus)
{
...
Retcode_T ConnectivityWLANTheNetwork_Enable(void)
{
	...
	exception = WlanNetworkConnect_Init(ConnectivityWLANTheNetwork_WlanConnectStatusCallback);
	...
	exception = WlanNetworkConfig_SetIpDhcp(ConnectivityWLANTheNetwork_NetworkIpConfigStatusCallback);
	if(exception == NO_EXCEPTION)
	{
