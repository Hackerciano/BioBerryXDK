...
#include "ConnectivityWLANTheNetwork.h"
#include "MitaTime.h"
...
Retcode_T Mita_initialize(void* userParameter1, uint32_t userParameter2)
{
	...
	// setup resources
	exception = ConnectivityWLANTheNetwork_Setup();
	if(exception == NO_EXCEPTION)
	{
		printf("[INFO, %s:%d] setup ConnectivityWLANTheNetwork succeeded\n", __FILE__, __LINE__);
	}
	else
	{
		printf("[ERROR, %s:%d] failed to setup ConnectivityWLANTheNetwork\n", __FILE__, __LINE__);
		return exception;
	}
	
	exception = ConnectivityHttpRestClientHttpBackEnd_Setup();
	if(exception == NO_EXCEPTION)
	{
		printf("[INFO, %s:%d] setup ConnectivityHttpRestClientHttpBackEnd succeeded\n", __FILE__, __LINE__);
	}
	else
	{
		printf("[ERROR, %s:%d] failed to setup ConnectivityHttpRestClientHttpBackEnd\n", __FILE__, __LINE__);
		return exception;
	}
...
Retcode_T Mita_goLive(void* userParameter1, uint32_t userParameter2)
{
	...
	exception = ConnectivityWLANTheNetwork_Enable();
	if(exception == NO_EXCEPTION)
	{
		printf("[INFO, %s:%d] enable ConnectivityWLANTheNetwork succeeded\n", __FILE__, __LINE__);
	}
	else
	{
		printf("[ERROR, %s:%d] failed to enable ConnectivityWLANTheNetwork\n", __FILE__, __LINE__);
		return exception;
	}
	exception = ConnectivityHttpRestClientHttpBackEnd_Enable();
	if(exception == NO_EXCEPTION)
	{
		printf("[INFO, %s:%d] enable ConnectivityHttpRestClientHttpBackEnd succeeded\n", __FILE__, __LINE__);
	}
	else
	{
		printf("[ERROR, %s:%d] failed to enable ConnectivityHttpRestClientHttpBackEnd\n", __FILE__, __LINE__);
		return exception;
	}
